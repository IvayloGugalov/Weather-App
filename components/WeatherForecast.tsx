...
import { RiWindyFill, RiErrorWarningLine,RiHeartAddFill, RiArrowLeftSLine, RiArrowRightSLine } from 'react-icons/ri'
...
function GetIcon({weather, timezone}: {weather: string | undefined, timezone: string}) {
...
const WeatherForecast = ({ search }: {search: string}) => {
  ...
  // const { weather, loading, error } : {weather:Weather, loading: boolean, error: string} = useFetchCityWeather(search)
      ...
      {weather && (
        <>
          <div className='flex flex-col justify-center items-center transition-all duration-500' id='data-container'>
            <h1 className="text-3xl font-semibold tracking-tight text-gray-700 dark:text-white">{search}</h1>
